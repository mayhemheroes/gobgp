...
if [ "$FIRST_CRASHED_ANY" = true ]; then
    echo "Initial triage: CRASHED in at least one attempt (MTV exit code: $FIRST_TRIAGE_EXIT_CODE)"
else
    echo "Initial triage: PASSED all 5 attempts (MTV exit code: $FIRST_TRIAGE_EXIT_CODE)"
fi
...
if [ "$SECOND_CRASHED_ANY" = true ]; then
    echo "Second triage: CRASHED in at least one attempt (MTV exit code: $SECOND_TRIAGE_EXIT_CODE)"
else
    echo "Second triage: PASSED all 5 attempts (MTV exit code: $SECOND_TRIAGE_EXIT_CODE)"
fi
